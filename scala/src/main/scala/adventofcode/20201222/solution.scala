...
object problem{	
	...
	case class Player(id:Int, initialCards: Seq[Int]){
		...
		override def clone = Player(id, cards.clone.toSeq)
		def total = if(cards.length == 0) 0 else cards.zipWithIndex.map(item => (cards.length - item._2) * item._1).sum
	...
	case class Game(players:Seq[Player], part:Int = 1){
		...
		def playRound():Boolean = {      
		  if(result != 0) return false else configs += config
		  val ( card1 , card2 ) = ( player1.draw , player2.draw )
		  var winner = if(card1 > card2) 1 else if(card2 > card1) 2 else 0      
		  if(part == 2) if( ( card1 <= player1.cards.length ) && ( card2 <= player2.cards.length ) ) winner = Game( Seq(
				Player(1, player1.cards.slice(0, card1).toSeq), Player(2, player2.cards.slice(0, card2).toSeq) ) ).playGame()
